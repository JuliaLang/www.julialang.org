...
To continue our case study of Julia's `Rational` type, here are the conversions declared in [`rational.j`](https://github.com/JuliaLang/julia/blob/master/j/rational.j), right after the declaration of the type and its constructors:
...
Here are some of the the catch-all method definitions given in [`promotion.j`](https://github.com/JuliaLang/julia/blob/master/j/promotion.j):
...
There are definitions of catch-all promotion methods for a number of other arithmetic and mathematical functions in [`promotion.j`](https://github.com/JuliaLang/julia/blob/master/j/promotion.j), but beyond that, there are hardly any calls to `promote` required in the Julia standard library.
...
For example, recall that [`rational.j`](https://github.com/JuliaLang/julia/blob/master/j/rational.j) provides the following outer constructor method:
...
The curious reader can read the code in [`promotion.j`](https://github.com/JuliaLang/julia/blob/master/j/promotion.j), which defines the complete promotion mechanism in about 35 lines.
