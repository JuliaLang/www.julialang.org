...
- Modernize the documentation for `IntervalArithmetic.jl`.
...
- Several of the packages currently have rather minimal documentation, in particular `IntervalOptimization.jl`, `IntervalConstraintProgramming.jl` and `TaylorModels.jl`. These need full tutorials and "how-to's" for solving example problems, as well as reference documentation. Preferably they should also be compared their use to other techniques for solving similar problems. Potential pitfalls should also be highlighted.

- Design a web page for the `JuliaIntervals` suite of packages. This web page should provide information about the various packages in the suite, as well as related packages in the Julia ecosystem, showing what they can be used for and exhibiting interesting examples. The web page should also contain a guide to orient a user towards the relevant package for their needs, for example using some kind of decision tree.
