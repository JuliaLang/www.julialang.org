...
Julia has always been a functional language in the technical sense: functions are values that can be passed to and from other functions (i.e. [higher-order functions](https://en.wikipedia.org/wiki/Higher-order_function)) and there is full support for [lambdas](https://en.wikipedia.org/wiki/Anonymous_function).
...
When many packages extend the same generic functions, it's *very* common for the methods added to have some ambiguous overlap.
...
This syntax translates to `broadcast(f, A1, A2, ...)`, where `broadcast` is a higher-order function, introduced in 0.4, which generically implements the kind of broadcasting behavior found in Julia's "dotted operators" such as `.+`, `.-`, `.*`, and `./`.
...
This may seem a little bit uglier, but it's more consistent than choosing and essentially arbitrarily set of functions to pre-vectorize.
But beyond that, as I'll explain below, this approach can have major performance benefits as well.

To give a more conrete sense of what this syntax can be used for, consider the `clip` function defined previously for real arguments.
...
This computation avoids allocating any intermediate arrays and performs the entire vectorized computation in one fell swoop into the result array.
...
The speed gain is marginal, but allocations are reduced by a factor of 3 as predicted – only the output array is allocated, no temporaries are.
...
The fact that this *still* isn't that much faster than the version that allocates several temporary and an output array is primarily a testament to how fast and carefully optimized Julia's allocator is;
eventually, if you're allocating, however, the garbage collector does kick in and then the piper gets paid.
...
In other words, instead of `.+` being a function as it is now, with it's own potentially unrelated behavior independent of `+`, it sill simply be the vectorized form of `+`.
...
In this case, the allocation saved is trivial, but it transforms this code from *O(n)* in the number of samples to *O(1)*.
...
For example, in `A[1,:]` the first slice is scalar, the second is not; in `A[:,2]` the second slice is scalar, the second is not; in `A[3,4]` both slices are scalar.
...
Unfortunately, there's no way to make this change without breaking some code – we apologize in advance for the inconvenience, and we hope you find the improvement to be worthwhile worthwhile.
...
Sometimes we can't do that so we just bite the bullet and break code with an error – so rude! But changing slices to views wouldn't break code with an error, it would just silently cause code to produce different, incorrect results.
...
- Array views now support all forms of indexing supported by arrays, whereas previously there some more complex forms of indexing were not supported for views.
...
The `ASCIIString` and `UTF8String` types have been merged into a single `String` type, and the `UTF16String` and `UTF32String` and other types and functions related to string encodings other than UTF-8 have been moved into the `LegacyStrings` package, which keeps the same implementations as 0.4.
In the future, better support for different string encodings will be developed under the `StringEncodings` package.

- Most functionality related to prime generation, primality checking and combinatorics, has been moved into one of the two external packages: `Primes` or `Combinatorics`.
...
With support for these architectures, Julia is becoming a powerful platform mainframes and embedded computing.
...
Note that this is true concurrent execution like C++, Go or Java – Julia is compiled so there's no [global interpreter lock](https://en.wikipedia.org/wiki/Global_interpreter_lock) preventing different Julia threads from doing real computation at the same time.

- Interactive debugging support has been a weak spot in the Julia ecosystem for some time, but not any more: on a vanilla build of Julia 0.5, you can install the `Gallium` package to get a full-fledged, high-performance debugger.
Set breakpoints, set through code, examine variables and stack frames.
