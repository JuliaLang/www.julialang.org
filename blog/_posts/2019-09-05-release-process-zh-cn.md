...
从事软件开发的行家里手们对开发流程与节奏如此了若指掌，以至于他们将其精髓**内化**（internalize）并以为人人都懂得这些“浅显”的道理。
...
如何大家觉得这篇文章颇有益处，我们会考虑将其变成一份官方文档。
...
- 次要版本每**四月**发布一次，也就是每年发布三次。
  ...
  在冻结当天，我们为次要版本建立`release-1.3`分支（译者注：此文写作时的版本分支）。
...
- 依据SemVer，主要版本可以大刀阔斧地改动。
  ...
  大部分用户级别代码会在Julia `2.0`版本（译者注：此文写作时的未来主要版本）中完整的保留下来。
...
另一些风险承受力低的用户希望使用久经考验，日臻完善的版本。
还有些用户介于这两个极端间的某处。
大致可以把大多数用户根据风险承受力分为下面四类：

1. **高风险承受力**（high risk tolerance）：“人生只有一次，我在master分支上翩翩起舞。况且，master分支已经不像以前那样不稳定了。现在master只偶尔抽风，就算抽风了，我也可以帮忙解决它。”
...
3. **低风险承受力**（low risk tolerance）：“我保守，厌恶风险。我使用当前的LTS分支，因为它已经经历了充分的测试。当LTS分支更替时，我将升级到新的LTS分支。因为新的LTS分支成为长期支持时已经经历了数个补丁版本，所以bug应该已经被修复，第三方库不兼容问题应该也已经被修正。”
...
我们将从实践中学习LTS分支在独当一面前会滞后稳定分支多少版本。

## 发布流程

我们已经讨论了各种版本以及它们所允许的修改，但我们还没深入讨论这些版本的发布流程。
在这一节里，我将概述如何从`master`上的新特性到次要版本的发布，再到为次要版本发布补丁。
在这节，“bug”一词不仅指代传统意义上错误的代码，也同时指代性能问题（运行效率不可接受之低的代码）。
在Julia语言中，性能至关重要，我们经常将性能问题视为不可绕过的bug。
以下是一连串围绕`x.y.0`次要版本的事件：
...
![](http://www.zhengwenjie.net/cheatsheet/release-process.svg)

一眼望去，你就能发现这是一条道阻且长的征途。
尤其是安定化阶段，它的费时忽长忽短，从几周到数月不等，难以预料。
...
为了解决这一矛盾，我们想了一个好主意。
如果我们同时开展一个版本的安定化和下一版本的开发，我们就有望如期完成版本迭代。
...
这个操作方法也意味着`master`分支永远开放用于接受新特性，而不会像不稳定分支那样在安定化阶段冻结。

由于开发与安定化的时间重叠，如果版本候选过程耗时过长，很有可能`x.y.0`的最终版本将在`x.(y+1).0`特性冻结时发布。
一个最好的例子便是`1.2.0`版本和`1.3.0`版本。
虽然这在discourse上引起了一些困惑和惊愕，但这种副作用是维持可预测发布周期所必要的。
`1.2`版本的安定化阶段不寻常的长，但这并没有什么好奇怪的。
我们时时检视我们的开发流程，反思如何改进。
一个可能的改进是更频繁地调用PkgEval以及自动化这一过程。
这样我们就能尽早地知道何时我们破坏了与第三方库的兼容性。
调用PkgEval越早，调用PkgEval越频繁，我们也就越容易锁定破坏兼容性的变动。
如果有人愿意帮助改善Julia的发布流程，最有效的途径就是改善PkgEval，而且这也不需要什么高深的技术知识。
...
- beta版本和alpha版本很相似，但更完善，含有较少的bug。我们只在`0.6`和`0.7`版本发布过beta版本（两者之前都已有alpha版本）。

- 候选版本说道：“这下总算快完成了，请测试并告诉我们是否有bug。如果不这样做，我们发布的版本可能会含有影响你的应用的bug。”候选版本，只要没有阻碍发布的bug，随时都会成为下一个正式版本。
...
同时，这个bug每波及一个活跃分支，这个PR就会被贴上相应的`backport x.y`的**GitHub标签**（label）。
...
如果你想助我们一臂之力，自动化并精简补丁版本发布流程是另一个高影响力的工作。
