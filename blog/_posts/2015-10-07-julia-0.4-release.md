...
JuliaCon 2015 was held in June, and >60 talks are [available to view](https://www.youtube.com/playlist?list=PLP8iPy9hna6Sdx4soiGrSefrmOPdUWixM).
...
- [Tuple type improvements](https://github.com/JuliaLang/julia/issues/10380): the type tuple `(A,B)`
  now written `Tuple{A,B}`. One notable consequence of this work is that fixed-size aggregae fields
  may now be written `field::NTuple{N,T}` (`N`umber of elements of given `T`ype).
