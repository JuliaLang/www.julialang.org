...
对于 Julia 的默认全局随机数生成器（`rand()` 和相关函数），我们采取的是将它独立于特定线程。
...
另一个不错的例子是[异常处理个性（personality）丢失][missing exception handling personality]。
事后看来，这本来很简单，但被两个因素所困：
第一，故障模式导致内核中断我们的进程，但我们无法在调试器中拦截；
第二，故障是由一个看似不相关的改变导致的。
所有 Julia 的栈帧都有一个异常处理个性集（personality set），因此问题只能出现在任何 Julia 框架之外的运行时系统中，
