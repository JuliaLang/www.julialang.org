---
layout: post
title:  发布Julia中的可组合的多线程并行(Simplified Chinese)
author: Jeff Bezanson (Julia Computing), Jameson Nash (Julia Computing), Kiran Pamnany (Intel)
---


软件性能越来越依赖于利用多个处理器核心，
摩尔定律的[免费午餐][free lunch]已然结束。
我们 Julia 社区在关注性能方面享有盛誉。
为了追求它，我们已经为多进程、分布式编程和 GPU 构建了许多功能，
但多年来，我们知道我们还需要一个关于可组合多线程的好故事。
今天，我们很高兴地宣布这个故事的一个新的重要篇章。
我们正在发布一个 Julia 的全新的线程接口：通用任务并行。
其灵感来自这些并行系统，如 [Cilk][]，[Intel Threading Building Blocks][] (TBB) 以及 [Go][]。 
任务并行现在可在 v1.3.0-alpha 版本中提供，这是 Julia 1.3.0 的早期预览版，可能在几个月内发布。
...
这一并行模型有许多奇妙的特性。我们认为它有点像垃圾回收：
使用 GC，你可以自由分配对象而无需担心何时以及如何释放对象。
而使用任务并行，您可以自由生成任务（可能有数百万个任务），而无需担心它们在哪运行。

这一模型非常轻量，而且不用关心底层细节。
您无需显式启动和结束线，甚至不需要知道有多少处理器或线程（尽管您仍可以按需得知）。

该模型是可嵌套和可组合的：
您可以启动并行任务，这些任务调用库函数，这些函数本身也可以启动并行任务，一切都会正常运行。
您的 CPU 不会因线程过多而饱和。
此特性对于高级语言至关重要，因为它们由库函数完成大量工作。
您只需要自由编写所需的任何代码（包括并行代码），而无需担心它调用的库是如何实现的
（目前仅适用于 Julia 代码，但将来我们计划将其扩展到 native 库（如 BLAS））。

事实上，这是我们对此次发布感到兴奋的主要原因：
从现在开始，多核并行的能力将为整个 Julia 生态系统赋能。

## 一些历史

这个新功能最令人惊讶的方面之一是它的历史。
从一开始（甚至从 0.1 版本之前），Julia 就提供对称协程的类型，该类型用于基于事件的 I/O。
因此，我们始终在语言中具有*并发*单元（独立的执行流），只是它不是并行的（同时的执行流）。
我们需要基于线程的并行性，因此在 2014 年（大致是 0.3 版本），我们着手进行漫长的重构过程来使代码线程安全。
Yichao Yu在垃圾回收器和线程本地存储性能方面投入了一些特别令人印象深刻的工作。
作者之一（Kiran）设计了一些基础结构，用于调度多个线程和管理原子数据结构。

大约两年后，在版本 0.5 中，我们发布了具有“实验”状态的宏 `@threads for` ，
该宏可以处理在所有核心上运行的简单并行循环。
尽管这不是我们想要的最终设计，但它做了两项重要的工作：
它让 Julia 程序员可以利用多个内核，并提供测试用例来清除运行时中与线程相关的Bug。
然而，这个早期版本的 `@threads` 有一些巨大限制：
 `@threads` 循环不能嵌套：如果调用的函数递归调用`@threads`，这些内部循环将只占用调用它们的CPU。
它还与我们的 `Task` 和 I/O 系统不兼容：
您无法在线程循环内进行任何 I/O 或在 `Task` 切换。
...
以研究如何进行集成。Julia 运行时带来了许多额外的功能，如垃圾回收和基于事件的 I/O，因此这并不简单。
令人失望的是，搞定新系统的时间比预期的要长得多——将近两年，尽管对于一个复杂的软件项目来说并不罕见。
...
现有的 `@threads for` 仍然有效，而且现在完全支持 I/O：
...
废话少说，让我们尝试一些嵌套的并行。
年年都要说的示例是并归排序，它将其输入分成两半，并递归地对各部分排序。
...
function psort!(v, lo::Int=1, hi::Int=length(v))
    ...
    half = @spawn psort!(v, lo, mid)  # 排序低半部分的任务将会与
    psort!(v, mid+1, hi)              # 当前排序高半部分的代码并行运行
                                      # 
    wait(half)                        # 等待低半部分完成
...
考虑在三个线程上运行的这种双路分治算法会让人有点头疼！
我们认为，这有助于强调此接口的“自动”并行。
...
单独的线程安全的条件变量 `Threads.Condition` 现已引入。
这需要是一个单独的类型，因为使用线程安全的条件变量需要获取锁。
...
他们只用于极少数情况：(1) 只有线程，而不是任务需要同步，(2) 您已知锁将只持有很短的时间。

`Threads` 模块还提供 `Semaphore`（信号量）和 `Event`（事件）类型，它们具有其标准定义。

### 线程局部状态

Julia 代码自然倾向于纯函数式（没有副作用或可变性），或者只使用局部可变性，
因此在许多情况下迁移到完全的线程安全应该很容易。
...
我们使用 Julia 的默认全局随机数生成器（`rand()`和相关函数）需要保证其特定于线程。
首次使用时，每个线程将从系统熵创建一个默认 RNG 类型(当前为`MersenneTwister`)的独立种子实例。
然后，影响随机数状态（`rand`、`randn`、`seed!`等）的所有操作将仅对当前线程的 RNG 进行操作。
这样，种子和使用随机数的多个独立代码序列将按预期独立工作。
...
不建议使用它，因为很难预测程序需要多少堆栈空间，例如编译器或其他调用库。
...
如果我们允许调度器在最近阻塞的任务的上下文中运行，就会减少开销。
这可以正常工作，但这意味着任务可以以奇怪的中间状态存在，
该状态被认为未运行，但实际上正在运行调度器。
这意味着，我们可以将任务从调度器队列中拉出，以意识到我们根本不需要切换。
...
此 alpha 版本引入了 `@spawn` 构造，但不打算最终确定其设计。
