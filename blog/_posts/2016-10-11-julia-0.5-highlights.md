...
This can be a big win if we're able to reuse the same output array for multiple computations.
...
In other words, instead of `.+` being a function as it is now, with it's own potentially unrelated behavior independent of `+`, when you write `A .+ B` it will mean `broadcast(+, A, B)`.
Also, dotted operators will participate in the same syntax-level fusion as other vectorized calls, so expressions like `exp.(log.(X) .+ log.(Y))` will translate into a single call to broadcast:
