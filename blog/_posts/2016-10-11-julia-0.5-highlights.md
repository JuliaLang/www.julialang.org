...
But beyond that, as I'll explain below, this approach can have performance benefits as well.
...
With the vectorized call syntax, however, the array of matrix of pairs never has to be constructed, nor does the messy code to apply `clip` to each element of `B` with the corresponding values of `lo` and `hi` need to be written.
...
In this form, the code allocates two temporary intermediate arrays – one to store `abs(X)` and one for `abs(Y)`.
...
In other words, `max.(abs.(X), abs.(Y))` becomes
...
As you can see, allocations are reduced by a factor of 3 as predicted – only the output array is allocated, no temporaries are.
The computation is also 4.3 times faster due to avoiding allocation and performing the entire computation in a single pass over the arrays.
...
With in-place vectorized assignment, we can fill the pre-allocated array, `Z`, without doing any allocation (the 96 bytes is an artifact), and do so 7.3 times faster than the original vectorized computation.
...
Dotted operators like `.+` and `.*` will cease to be their own independent operators and simply become the vectorized form of the corresponding scalar operators, `+` and `*`.
...
Wrapping this in the `@time` macro shows that this expression allocates 2.2 KB of memory, mostly for the array of sums.
But allocating an array is unnecessary here:
the minimum and maximum can be computed over streamed data simply by keeping the largest and smallest values seen so far.
...
Significantly, the allocation of this version doesn't depend on the size of the `change` array anymore – it will always be just 208 bytes, even if `change` holds a trillion tuples.
...
More concise, just as clear, and significantly more efficient:
...
As you can see from these benchmarks, the version with an array comprehension uses twice as much memory and is 50% slower than constructing the same `IntSet` using a generator expression.
...
- `k => v` as a standalone syntax for a `Pair` object;
...
So when you sliced a column out of a matrix the result was 1-dimensional, but when you sliced a row the result was a 2-dimensional row matrix:
...
- Most functionality related to prime generation, primality checking and combinatorics, has been moved into one of two external packages: [`Primes`](https://github.com/JuliaMath/Primes.jl) and [`Combinatorics`](https://github.com/JuliaMath/Combinatorics.jl).
...
Julia 0.5 introduced initial support for [Power] systems, a development which has been supported and driven by IBM.
We will be expanding and improving support for various architectures going forward.
