...
[分散式（多節點）平行運算](http://www.stochasticlifestyle.com/multi-node-parallelism-in-julia-on-an-hpc/)
以及[精密的事件處理](https://docs.juliadiffeq.org/stable/features/callback_functions.html)。
...
以及參考我們的[DifferentialEquations.jl 微分方程教學手冊](https://docs.juliadiffeq.org/stable/tutorials/ode_example.html)。
...
[手冊](https://docs.juliadiffeq.org/stable/)以獲得更多進階的使用方式。
...
由於 [DifferentialEquations.jl 同樣也可以處理隨機微分方程](https://docs.juliadiffeq.org/stable/tutorials/sde_example.html) (也是目前唯一一個包含剛性與非剛性隨機微分方程解算器的函式庫)，
...
[DifferentialEquations.jl 也提供了敏感性分析的實作](https://docs.juliadiffeq.org/stable/analysis/sensitivity.html)。
...
舉例來說，具有非連續性（[事件](https://docs.juliadiffeq.org/stable/features/callback_functions.html)）的常微分方程並不符合可微分這個假設。
